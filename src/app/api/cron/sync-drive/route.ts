...
/**
 * GET : Appelé par le cron Vercel - déclenche la synchronisation
 * Vercel Cron Jobs envoient des requêtes GET avec Authorization: Bearer <CRON_SECRET>
 */
export async function GET(request: NextRequest) {
  try {
    ...
    if (!token || token !== process.env.CRON_SECRET) {
      console.error('❌ Tentative d\'accès non autorisé au cron sync-drive (GET)');
      return NextResponse.json(
        { error: 'Unauthorized' },
        { status: 401 }
      );
    ...
    console.log('🔄 [CRON GET] Démarrage de la synchronisation automatique...');
    ...
    console.error('❌ Erreur globale sync-drive (GET):', error);
